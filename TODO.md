...
## additional todo based on thoughts while developing 
- Rapid double-clicking “Add to cart” could create duplicate optimistic updates before the server responds. Disabling the button while the server action is pending would harden this..


## changes we made to official repo

- non optimistic loading 

## We made changes to cart-context.tsx

## from AI:
Why it feels “non-optimistic” right now

1. Every component that calls `useCart()` creates its **own** `useOptimistic` state.
   • `useCart()` fetches the cart promise, then runs  
     `const [optimisticCart, updateOptimisticCart] = useOptimistic(...)`.

2. Because that state lives inside the hook, each caller
   ( `AddToCart`, `CartModal`, etc.) keeps an **independent copy** of the cart.

3. When you press the “Add to cart” button the sequence is:
   a. `AddToCart`’s `action` runs on the client.  
      It calls `addCartItem(...)`, which updates **its own** optimisticCart.  
      b. `CartModal` and the badge in the navbar are *different* components,
      with *different* `useCart()` instances, so they don’t see that change.
   c. After the server action (`addItem`) finishes, the page re-fetches the
      real cart and everybody re-mounts with the updated data – that’s the delay
      you’re observing.

4. Inside the cart modal the quantity buttons feel instant because the buttons
   and the UI that displays the quantity share the *same* `useCart()` instance
   (they live in the same component tree), so they update together.

Root cause

`CartProvider` only puts `{ cartPromise }` in context.  
The optimistic cart state and the mutating functions are **not** stored in that
context, so they are not shared across the app.

Fix: lift the optimistic state into the provider

Move `useOptimistic` (and the helper functions) from the hook to the provider,
store the current cart and the mutators in context, and make
`useCart()` just read that context. With one central state, every component
sees the optimistic update immediately.
...
export function CartProvider({
  children, cartPromise
}: { children: React.ReactNode; cartPromise: Promise<Cart | undefined>; }) {
  const initialCart = use(cartPromise);
  const [cart, dispatch] = useOptimistic(initialCart, cartReducer);
...
Now `AddToCart`, `CartModal`, the badge, etc. all consume the **same**
optimistic state:

```tsx
const { addCartItem } = useCart();   // shared
```

Pressing “Add to cart” will immediately:
• bump `cart.totalQuantity`,  
• open the modal via the effect in `CartModal`,  
• update the badge count,

all before the server returns.



## products page

A focused checklist to finish the Product Detail Page to match the target UI.

### Structure & composition
- [ ] Create product component folder under `src/components/product/`:
  - [ ] `Hero.tsx` (server layout wrapper)
  - [ ] `Gallery.tsx` (client; carousel + dots)
  - [ ] `Copy.tsx` (server; title + one‑line blurb)
  - [ ] `PurchasePanel.tsx` (client; wraps size selector, size guide link, add‑to‑cart)
  - [ ] `SizeSelector.tsx` (client; radio group, disabled/out‑of‑stock support)
  - [ ] `Info.tsx` (client; accordion sections)

- [ ] Update `src/app/products/[handle]/page.tsx` to compose `Hero` + `Info`:
  - [ ] Keep `ProductProvider` around the page
  - [ ] Keep JSON‑LD block
  - [ ] Grid: full‑bleed rounded gallery panel; right rail `PurchasePanel` on desktop; stacked on mobile

### Gallery (client)
- [ ] Rounded, neutral background panel; uses `next/image`
- [ ] Carousel with swipe/keyboard + dots; initial slide preloaded
- [ ] Sync selected slide with `useProduct().updateImage(index)` (optional URL sync)
- [ ] Provide alt text and show a skeleton while loading

### Copy (server)
- [ ] Render `h1` title
- [ ] One‑line blurb from `product.seo.description || product.description`

### Purchase panel (client)
- [ ] Derive selected variant from `product.variants` using `useProduct().state`
- [ ] Render existing `AddToCart` from `src/components/cart/add-to-cart.tsx`
- [ ] Show price at the right of the button (selected variant price; fallback to min range)
- [ ] Disable while no variant is selected or product not available
- [ ] Prevent rapid double clicks (disable while server action pending)
- [ ] Include `SizeGuideLink` beneath the selector

### Size selector (client)
- [ ] Read `size` option from `product.options` (case‑insensitive)
- [ ] Render radio group from Shopify values (e.g., xs–xl)
- [ ] Disable values that only exist on variants with `availableForSale === false`
- [ ] On select: `updateOption('size', value)` then `useUpdateURL(nextState)` to sync `?size=`
- [ ] If only one variant exists, auto‑select it
- [ ] Accessibility: `role="radiogroup"`, `role="radio"`, `aria-checked`, keyboard focus styles

### Size guide link
- [ ] Default: smooth‑scroll to `#find-your-size` in the accordion
- [ ] Optional: later switch to modal without changing the API

### Info accordion (client)
- [ ] Implement `Info.tsx` with sections:
  - [ ] Description (render `product.descriptionHtml`)
  - [ ] FindYourSize (target of size‑guide link; container `id="find-your-size"`)
  - [ ] CareInstructions
  - [ ] DeliveryAndReturns
- [ ] Plus/minus affordance and smooth expand/collapse
- [ ] Focus management and correct `aria-expanded`/`aria-controls`

### Sections context integration
- [ ] On product page mount, call `setSections()` with:
  - [ ] `[{ id: 'description', name: 'description' }, { id: 'find-your-size', name: 'find your size' }, { id: 'care-instructions', name: 'care instructions' }, { id: 'delivery-and-returns', name: 'delivery and returns' }]`
- [ ] Ensure `SizeGuideLink` scrolls to `#find-your-size`

### Layout & styling
- [ ] Match rounded corners and spacing from mocks
- [ ] Mobile: stacked flow (gallery → copy → purchase → accordion)
- [ ] Desktop: centered gallery, left copy, right purchase panel
- [ ] Button styling with right‑aligned price inside the button

### Accessibility
- [ ] Ensure a single page `h1`
- [ ] Radiogroup semantics and visible focus for SizeSelector
- [ ] `aria-live` status message for add‑to‑cart (already present)
- [ ] Accordion and links keyboard accessible

### Performance
- [ ] Lazy‑load non‑primary images and below‑the‑fold sections
- [ ] `sizes` attribute tuned per breakpoint; only first image `priority`
- [ ] Memoize disabled‑map and selected variant derivation

### Edge cases
- [ ] No `size` option: hide selector; allow direct add‑to‑cart
- [ ] All variants sold out: show disabled “Out of stock” button
- [ ] Invalid deep‑link `?size=` gracefully resets selection

### Verification
- [ ] Selecting a size updates URL and UI
- [ ] Add‑to‑cart instantly updates cart badge and opens cart modal (optimistic)
- [ ] Refresh with `?size=` preserves selection
- [ ] Desktop and mobile layouts match mocks
